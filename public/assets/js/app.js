            ...
            <div class="container">
                <div class="mt-5">
                    <div class="d-style btn btn-brc-tp border-2 bgc-white btn-outline-blue btn-h-outline-blue btn-a-outline-blue w-100 my-2 py-3 shadow-sm">
                        <!-- Basic Plan -->
                        <div class="row align-items-center" v-for="album in $root.albumList">
...
const editAlbumComponent = {
    async mounted() {
        ...
        axios.get(`https://moat.ai/api/task/`, {headers: {
    ...
    methods: {
        saveAlbum: function(e) {
            let self = this;
            self.$root.submitForm(e, `v1/album/${id}`, function(response) {
                toastr.success(response.data.message, 'Album', toastrOptions);
                self.$route.push({'name': 'albums'});
            });
        }
    }
};
